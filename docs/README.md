...
networks. It is fully self-contained: [`vis-network`](https://visjs.github.io/vis-network/) is
bundled locally as `vis-network.min.js`, so the viewer works offline (double-click
`index.html`) with no external / CDN dependency.
